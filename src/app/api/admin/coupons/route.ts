...
export async function GET() {
  ...
  const data = await Promise.all(
    coupons.map(async (c) => {
      ...
      const arrayCount = c.usedBy?.length ?? 0;
      const actualUsage = Math.max(c.usageCount ?? 0, orderCount, arrayCount);
...
export async function POST(req: NextRequest) {
  ...
  const { code, description, type, value, minOrderAmount, maxDiscountAmount, usageLimit, userLimit, startDate, endDate, isActive } = body;
